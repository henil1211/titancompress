    ...
    <header class="bg-white shadow-md sticky top-0 z-50" x-data="{ mobileMenuOpen: false }">
        <div class="max-w-7xl mx-auto px-6 lg:px-12 py-4 flex justify-between items-center">
            <a href="/" class="flex items-center space-x-3">
                <div class="w-10 h-10 lg:w-12 lg:h-12 bg-industrial-orange rounded-lg flex items-center justify-center font-black text-xl lg:text-2xl text-white shadow-md">T</div>
                <div class="flex flex-col">
                    <span class="text-xl lg:text-2xl font-black tracking-tight text-industrial-blue leading-none">TITAN</span>
                    <span class="text-xs lg:text-sm font-bold tracking-[0.2em] text-industrial-orange leading-none">COMPRESS</span>
                </div>
            </a>
            ...
            <nav class="hidden lg:flex items-center space-x-8">
                ...
                <div class="relative group" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    ...
                    <!-- Dropdown -->
                    <div x-show="open" x-transition x-cloak class="absolute top-full left-0 mt-2 w-64 bg-white shadow-xl border border-slate-100 rounded-lg py-2 z-50">
                        @php
            ...
            <button @click="mobileMenuOpen = !mobileMenuOpen" class="lg:hidden text-industrial-blue p-2">
        ...
        <!-- Mobile Menu -->
        <div x-show="mobileMenuOpen" x-cloak x-transition class="lg:hidden bg-white border-t border-slate-100 absolute w-full shadow-2xl">
            <div class="px-6 py-4 flex flex-col space-y-4">
                <a href="/" class="text-base font-bold uppercase tracking-widest {{ request()->is('/') ? 'text-industrial-orange' : 'text-industrial-blue' }}">Home</a>
                <a href="{{ route('about') }}" class="text-base font-bold uppercase tracking-widest {{ request()->routeIs('about') ? 'text-industrial-orange' : 'text-industrial-blue' }}">About Us</a>
                <a href="{{ route('products.index') }}" class="text-base font-bold uppercase tracking-widest {{ request()->routeIs('products.*') ? 'text-industrial-orange' : 'text-industrial-blue' }}">Products</a>
                <a href="{{ route('compare.index') }}" class="text-base font-bold uppercase tracking-widest {{ request()->routeIs('compare.index') ? 'text-industrial-orange' : 'text-industrial-blue' }}">Compare</a>
                <a href="{{ route('contact') }}" class="text-base font-bold uppercase tracking-widest {{ request()->routeIs('contact') ? 'text-industrial-orange' : 'text-industrial-blue' }}">Contact</a>
                <div class="pt-4 border-t border-slate-100">
                    <a href="{{ route('rfq.show') }}" class="btn-industrial bg-industrial-orange w-full text-center py-3">REQUEST A QUOTE</a>
                </div>
            </div>
        </div>
    </header>
